...
class ImgManager extends Component {
    ...
    public function save($file, $name = null, $path = null, $id = null) {
        ...
        try {
            //$uid = \Yii::$app()->user->id;
            // $old = Image::findByAttributes(array( 'ug_id'=>(int)$uid));
            ...
            if ($id > 0) {
                ...
                $image = new \siripravi\slideradmin\models\Image();
    ...
    public function createVersion($id, $version) {
        if (isset($this->versions[$version])) {
            ...
            if ($image != null) {
                ...
                Picture::getImagine()->open($this->_basePath . '/files/images/' . $fileName)->thumbnail(new Box($options->width, $options->height))->save($path, ['quality' => 90]);
                return $path;
            } else
            //return $id;
